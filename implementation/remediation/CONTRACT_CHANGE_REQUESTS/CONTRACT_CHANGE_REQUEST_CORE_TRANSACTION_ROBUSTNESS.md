...
- **S-G15-1-1 = A:** افزودن `TRANSACTION_RETRYABLE` به `CoreErrorCode` و نگاشت `P2028` و `P2034` به آن.
- **S-G15-1-2 = 5000/10000ms:** `maxWait=5000ms` و `timeout=10000ms` به‌عنوان پیش‌فرض، با امکان override از راه متغیر محیط، و یکسان برای آزمون و runtime.
- **S-G15-1-3 = A:** هیچ retry خودکاری در Core نیست؛ caller روی `TRANSACTION_RETRYABLE` تصمیم می‌گیرد.
- **S-G15-1-4 = A:** `connection_limit` فقط در محیط آزمون به‌صورت صریح تنظیم می‌شود و runtime بدون تغییر می‌ماند.
