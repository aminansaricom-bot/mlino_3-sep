...
describe('G10a2 Core authority slice', () => {
  ...
  it('serializes concurrent platform revocations so only one can remove the last grant administrator', async () => {
    const a = await bootstrap.execute('platform-token', bootstrapInput(`${TEST_PREFIX}concurrency`));
    const owner = contextFor(a.foundingMembership, a.organization.id);
    const first = await memberships.create(owner, { organizationId: a.organization.id, identityProvider: 'test-idp', externalSubject: 'first' });
    const second = await memberships.create(owner, { organizationId: a.organization.id, identityProvider: 'test-idp', externalSubject: 'second' });
    await memberships.revokeByPlatform('platform-token', a.organization.id, first.id, 'remove first member');
    const secondGrant = await grants.issue(owner, second.id, 'permission_grant.issue', 'second admin');
    const results = await Promise.allSettled([
      grants.revokeByPlatform('platform-token', a.organization.id, secondGrant.id, 'concurrent revoke 1'),
      grants.revokeByPlatform('platform-token', a.organization.id, secondGrant.id, 'concurrent revoke 2'),
    ]);
