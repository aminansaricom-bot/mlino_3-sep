...
export function buildV2TrustBundle(descriptor: KeyStoreDescriptor, { version, revokedIds }: { version: string; revokedIds: string[] }) {
  const parsed = parseDescriptor(descriptor);
  if (typeof version !== 'string' || !version.trim() || !Array.isArray(revokedIds) || revokedIds.some((id) => typeof id !== 'string')) throw new Error('TRUST_BUNDLE_INVALID');
