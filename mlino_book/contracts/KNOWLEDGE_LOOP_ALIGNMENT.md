...
| **آفر** | `Offer` — D-53 | — |
| **پیشنهاد** | **فقط** `Recommendation` | `Offer` — و «پیشنهاد ویژه» برای کمپین |
| **نقش** | کارکرد سازمانی — D-55 | اجازه یا دسترسی |
| **مسئولیت** | انتظار رسیدگی — مسیریابی | اجازه |
| **اجازه** | اعطای ماندگار به **شخص** | نقش |
| **مجوزدهی** | عمل ثبت‌شده — `authorizedBy` · `decided_by` · `publishedBy` | «تایید» |
| **تایید** | «این **درست** است» — `confirmedBy` (ADR-0006) | مجوزدهی |
| **دسترسی** | آنچه می‌توان دید — AC-2 | نقش |
| **دسترس‌پذیری** | وضعیت عملیاتی آینده‌نگر و فاسدشدنی — D-56 · ⚠️ فقط یک پسوند با «دسترسی» فاصله دارد | «دسترسی» · اعتبار آفر |
| **اعتبار** (آفر) | بازه‌ای که شرایط آفر برقرار است — D-53 | دسترس‌پذیری |
...
**۵. (D-51)** لبه‌ی خط‌چین `Decision → Action`: تصمیم انسانی می‌تواند **مستقیم** به اقدام تبدیل شود، بی‌آنکه MLINO پیشنهادش داده باشد. پیش از D-51 این لبه وجود نداشت و حلقه‌ی یادگیری در عمل بسته نبود → [`ACTION_MODEL_ALIGNMENT.md`](ACTION_MODEL_ALIGNMENT.md)
...
✅ **حل شد — D-49**، در [`LEARNING_LOOP_ALIGNMENT.md`](LEARNING_LOOP_ALIGNMENT.md). در زمان نوشتن این بند اصلاحش نکردم چون طراحی فاز ۲ مصوب بود؛ توصیه‌ی آن زمان این بود:
...
**V2 فقط زیرمجموعه‌ی قابل‌انتشارِ لایه‌ی دانش پایدار را مصرف می‌کند** — توانمندی، و آفر (D-53 → [`OFFER_OWNERSHIP_ALIGNMENT.md`](OFFER_OWNERSHIP_ALIGNMENT.md)).
