...
| ۶ | تولیدکننده‌های Event | Data Ingestion (برون‌زا)؛ Learning & Feedback، Trust/Governance (نمونه‌های درون‌زا)؛ پرچم Materiality — **RESOLVED در Kernel v1.3، طبق ADR-00AD**: تولیدکننده‌ی سیگنال دامنه‌ی ثبت‌شده، از طریق IC-13 (رجوع به بخش ۵) |
...
- ~~**G3.**~~ **RESOLVED (فاز ۳D، طبق ADR-00AD و الحاقیه‌ی Kernel v1.3 §۶/§۲۰).** پرچم Materiality — تولیدکننده اکنون مشخص است: تولیدکننده‌ی سیگنال دامنه‌ی ثبت‌شده (Domain Signal Producer)، در لایه‌ی Feature/Domain Pack، از طریق IC-13. جزئیات: `governance/ADR-00AD-Opportunity-Producer-And-Materiality.md`.
