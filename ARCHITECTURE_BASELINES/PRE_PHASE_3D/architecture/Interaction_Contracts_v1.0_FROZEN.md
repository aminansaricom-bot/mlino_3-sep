# MLINO Interaction Contracts v1.0 (Frozen)

**Status:** Frozen — consumes the frozen Kernel Architecture Specification v1.2
...
3. **G3 — Materiality flag producer.** The Kernel defines the concept (§6, §22) but assigns no producing Capability. No Interaction Contract above touches Materiality because no Capability has standing to produce it yet. **Recommend:** ADR once a concrete use case (e.g., surfacing Materiality in Communication & Narrative) makes the missing ownership a real implementation blocker rather than a theoretical one.
