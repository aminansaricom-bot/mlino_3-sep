...
- ویرایش محصولات و ویترین (D-83): `MEDIA_STORE_DIR=/var/lib/mlino/media-store` در api.env؛ drop-in `/etc/systemd/system/mlino-api.service.d/media-store.conf` با `ReadWritePaths=/var/lib/mlino/media-store/media`؛ `chgrp -R mlino-api` روی همان پوشه، پوشه‌ها ۲۷۷۵ و فایل‌ها ۶۴۰؛ `GRANT INSERT ON catalog_items; GRANT INSERT, UPDATE, DELETE ON catalog_item_media TO mlino_api`؛ سپس seed-demo-members.
