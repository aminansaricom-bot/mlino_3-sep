...
try {
    ...
    $preCounts = foreach ($table in $tables) {
        ...
        "$table=$((Psql $tableSql)[0])"
    }
    Save-Log 'pre-table-counts.log' (($preCounts -join "`n") + "`n")
    $pubCount = [int](Psql 'SELECT count(*) FROM public.publications;')[0]
    $columnCount = [int](Psql "SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='publications' AND column_name='published_content';")[0]
    ...
    if ($pubCount -ne 0 -or $columnCount -ne 0 -or $triggerCount -ne 13) { throw 'HARD_STOP: publication preflight or trigger count failed' }

    $status = Run-Command 'migrate-status.log' { & $prisma 'migrate' 'status' '--schema' $schema }
    if ($status.Output -notmatch [regex]::Escape($migration)) { throw 'HARD_STOP: required migration is not pending' }
    $pendingNames = [regex]::Matches($status.Output, '\b20\d{12}_[A-Za-z0-9_-]+\b') | ForEach-Object { $_.Value } | Select-Object -Unique
    ...
    if ($postPubCount -ne 0 -or $columnType -ne 'jsonb' -or $constraint -ne 'publication_published_content_event_kind_check|validated=true' -or $postTriggerCount -ne $triggerCount -or $postCheckCount -ne ($checkCount + 1)) { throw 'POST_VERIFY_FAILED: constraints or counts' }
