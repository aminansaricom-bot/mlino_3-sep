...
export default function ExperiencePanel({data, records, storageFailed, onClose, onOpen, onChange, onToggle, onDiagnostics}: Props) {
  ...
  const tasks = [
    ...
    {label: 'ویترین سه مکان را ببین', done: viewed.length >= 3},
  ...
  return <section className="panel personal-panel" aria-label="فضای من">
    ...
    <div className="panel-body">
      <div className="personal-intro"><Icon name="compass"/><h2>شهرِ تو، انتخابِ تو</h2><p>مجموعه‌ها و پیشرفت کشف، روی همین مرورگر ذخیره می‌شوند.</p></div>
      {storageFailed && <p role="status" className="inline-note">ذخیره در مرورگر ممکن نیست؛ انتخاب‌ها فقط تا بستن این صفحه می‌مانند.</p>}
      {(['saved','later'] as const).map(key => <section key={key} className="collection-section"><h3>{key === 'saved' ? 'مکان‌های من' : 'بعداً ببینم'}</h3>
      ...
      <section className="discovery-tasks"><h3>قدم‌های کوچکِ کشف</h3><p className="muted">بر اساس مشاهده‌ی ویترین‌ها؛ به معنی بازدید حضوری یا پاداش مالی نیست.</p><progress value={tasks.filter(t=>t.done).length} max={3} aria-label="پیشرفت کشف"/>{tasks.map(task=><div className="task-row" key={task.label}><Icon name={task.done ? 'check' : 'compass'}/><span>{task.label}</span><small>{task.done ? 'انجام شد' : 'در انتظار کشف'}</small></div>)}</section>
